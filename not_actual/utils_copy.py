...
def spans_from_breakpoints(n_sent: int, bp_sent: List[int]) -> List[Tuple[int,int]]:
    """
    Преобразует индексы предложений, ПОСЛЕ которых ставится разрыв (0..n-2),
    в спаны [start, end] по предложениям (включительно).
    Инварианты:
      - пустой bp => один спан (0, n-1)
      - все bp строго возрастания в [0..n-2]
    """
