...
def valley_scores_symmetric(s: np.ndarray, k: int = 2) -> np.ndarray:
    ...
    for i in range(n):
        ...
        # V[i] = base * (m / max(1, k))
        weight = (m / max(1, k)) ** 0.5   # мягче штрафуем края
        V[i] = base * weight
...
def breakpoints(similarities: List[float], sentences: List[str], *,
                ...
                # --- новые «ручки» (можешь не трогать, дефолтов хватит) ---
                drop_mult: float = 1.0,  # было 1.0
                contrast_mult: float = 0.5,  # было 0.5
                rel_depth_alpha: float = 0.60,  # фильтр относительной глубины (0..1)
                min_side_frac: float = 0.50,  # требуемая доля от tau_drop для КАЖДОЙ стороны
                extra_nms_radius: int = 0  # добавка к базовому радиусу NMS
                ):
    """
    ...
    # --- робастный масштаб изменений и пороги ---
    diffs = np.diff(s)
    scale = mad(np.abs(diffs))
    tau_drop = drop_mult * scale  # было 1.00 * scale
    tau_contrast = contrast_mult * scale  # было 0.50 * scale
    ...
    for i in range(1, n - 1):
        ...
        # --- НОВОЕ: каждая сторона должна быть заметной,
        # чтобы отсечь случай "одна сторона мизерная" ---
        if min(dropL, dropR) < (min_side_frac * tau_drop):
            continue

        # контраст относительно окрестностей
...
def segment_with_textsplit(sentences: List[str], embeddings: np.ndarray, *,
    ...
    #     plt.show()
    #
    #     # График 2: карта разрывов по индексам предложений
    #     plt.figure()
